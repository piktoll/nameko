window.onload = function () {
    ...
    */
    ...
    var echoName = document.getElementById("echoname");
    var echoRomaja = document.getElementById("echoromaja");

    var checkbox = document.getElementById("family");
    var heading = document.getElementById("heading");
    var list = document.getElementById("list");
    let combo = formatter.format(korean[0].length * korean[1].length * korean[2].length);
    heading.innerText = "Korean Name Generator with " + combo + " possible name combinations";
    echoName.innerText = "Click Here!";
}

function generateName() {
    echoName.innerText = "";
    echoRomaja.innerText = "";

    if (checkbox.checked) {
        let pr = Math.floor(Math.random() * (korean[0].length - 1));
        echoName.innerText = korean[0][pr];
        echoRomaja.innerHTML = romanized[0][pr] + "&nbsp;";
    }

    let sr = Math.floor(Math.random() * (korean[1].length - 1));
    let tr = Math.floor(Math.random() * (korean[2].length - 1));
    
    echoName.innerHTML += korean[1][sr] + korean[2][tr];
    echoRomaja.innerHTML += romanized[1][sr] + "-" + romanized[2][tr];
}
...
function massGenerate(n) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (checkbox.checked) {
            list.innerHTML += korean[0][pr] + korean[1][sr] + korean[2][tr] + "&nbsp;<span class=\"romaja\">(" + romanized[0][pr] + "&nbsp;" + romanized[1][sr] + "-" + romanized[2][tr] + ")" + "</span><br>";
        } else {
            list.innerHTML += korean[1][sr] + korean[2][tr] + "&nbsp;<span class=\"romaja\">(" + romanized[1][sr] + "-" + romanized[2][tr] + ")" + "</span><br>";
        }
    }
}
